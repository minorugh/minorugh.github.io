...
# 毎日句会システム 試運転レポート
# changelog-20260416.md

作成：2026年4月16日  
対象：d_kukai 新システム（Docker試運転）  
ステータス：試運転中・本番移行前
...
- choice.cgi ログイン・選句・完了メッセージ 正常
- post.cgi 投句・編集・完了メッセージ 正常
- gosen.cgi 自動リセット処理（_datefrag.dat 日付変わり検知）正常
- 互選集計・画面表示 正常
- score/YYYYMM.html 生成・index.html コピー 正常
- bk.log 投句控追記 正常
- touku.dat シャッフル → entry.dat 生成 正常

---

## 試運転で発見されたバグと対応

### Bug 1：自動リセットが翌朝実行されなかった
**発生：** 翌朝 gosen.cgi にアクセスしてもリセットが走らず、
`_datefrag.dat` を手動で当日日付に書き換えると実行された。
実行後 `_datefrag.dat` が前日日付に戻ってしまう症状も確認。

**原因：** Docker 環境のタイムゾーンが UTC のため。
日本時間9時台にアクセスしても `localtime` は UTC 前日の0時台を返し、
`$kyou` が前日日付になっていた。
結果として「_datefrag.dat の値（前日）== $kyou（前日）」となりリセット条件を満たさず、
また実行後も前日日付を _datefrag.dat に書き込むという二重の問題が発生。

**対応：** 全CGI先頭に `BEGIN { $ENV{TZ} = 'Asia/Tokyo'; }` を追加。
本番 Xserver はタイムゾーンが Asia/Tokyo のため影響なし。
Docker 環境でも /etc/localtime を設定すれば根本解決するが、
コード側での対処を加えることで Docker / Xserver 双方で正しく動作する。

---

### Bug 2：互選結果と score への書き出しで同点句の並び順が異なる
**発生：** 5点・4点などの上位は一致するが、1〜2点の同点句の並びが
gosen.cgi 画面と score/YYYYMM.html で異なっていた。

**原因の変遷（修正のたびに問題が移った経緯）：**

**第1段階：** 当初 `_make_html` と `_show_gosen` がそれぞれ独自に集計・sort していた。
Perl のハッシュは同じキー・同じ値でも実行ごとに内部順序が不定なため、
2回 sort すると同点句の並びが変わる。

**第2段階（第1次修正）：** `sort { $KEKKA{$b} <=> $KEKKA{$a} || $a <=> $b }` という
第2ソートキー（清記番号昇順）を追加した。しかし `_make_html` と `_show_gosen` が
**別々に** sort していたため、同じアルゴリズムでも呼び出しタイミングが違えば
ハッシュの内部イテレーション順が変わり結果が一致しないケースが残った。

**第3段階（第2次修正）：** 集計処理を `_collect` サブルーチンに一本化し、
`_do_reset` → `_collect` → `_make_html` と `_show_gosen` → `_collect` という形にした。
しかしリセット時に `_do_reset` と `_show_gosen` がそれぞれ `_collect` を呼ぶため、
依然として2回集計・2回 sort していた。

**根本原因：** sort のアルゴリズムの問題ではなく「同じデータを2回 sort している」こと自体が問題。
Perl の sort はハッシュのキー列挙順（rand 的）に依存するため、
同じアルゴリズムでも2回実行すれば異なる結果になりえる。

**最終対応：**
- `_do_reset` が `_collect` を1回呼んで `($rows, $tpn, $spn)` を生成
- `_make_html` にその `$rows` を渡して HTML 書き出し
- `_show_gosen` にも同じ `($rows, $tpn, $spn)` を渡して再集計しない
- 通常アクセス（リセットなし）は `_show_gosen` が `_collect` を1回だけ呼ぶ
- 結果として `_collect`（= sort）は常に1回だけ実行され、両者が必ず一致する

---

### Bug 3：_do_reset 内で return が途中に挿入され手順4〜6が実行されていなかった
**発生：** Bug 2 の修正作業中（第3段階）に `return ($rows, $tpn, $spn)` を
手順3の直後に誤って挿入してしまった。

**影響：** 以下の処理が一切実行されない状態だった。
- 手順4：bk.log への投句控追記
- 手順5：touku.dat シャッフル → entry.dat 生成（翌日の清記が作られない）
- 手順6：touku.dat / data.dat のクリア（翌日の投句データが蓄積されたまま）

試運転中に発見・修正済み。`return` を手順6（全処理）の後ろに移動した。

**経緯のまとめ：** Bug 2 を「sort を共通化する」という方向で修正しようとした際、
`return` の挿入位置の確認が不十分だった。Python の str.replace による部分置換を
複数回重ねたことで意図しない位置に `return` が入った。
今後の修正作業では、サブルーチン全体を一度 view で確認してから編集することを徹底する。
...
- [ ] Bug 2・Bug 3 の修正後、翌朝の自動リセット動作を再確認
      （score書き出し・bk.log追記・entry.dat生成・データクリアの全手順）
- [ ] CGI::Carp `fatalsToBrowser` の削除（全3ファイル）
      動作が安定したら削除する。本番環境ではセキュリティ上必須。
- [ ] logview/ （投句控表示）の動作確認
- [ ] score/calssi.cgi（過去一覧カレンダー）の動作確認

### スコープ外（別途対応）
- w_kukai（若鮎句会）への転用：d_kukai 安定確認後
- s_kukai / m_kukai：グループB実装
...
| ファイル | 変更内容 |
|---|---|
| gosen.cgi | 自動リセット統合・makehtml.pl統合・TZ追加・_collect一本化・return位置修正 |
| post.cgi | 完了メッセージ内部化・use strict・平文照合統一・TZ追加 |
| choice.cgi | 完了メッセージ内部化・use strict・平文照合統一・FAQ復元・TZ追加 |
| makehtml.pl | 廃止（gosen.cgi に統合） |
| reset.cgi | 廃止（gosen.cgi に統合） |
| mes_post.html | 廃止（内部出力に変更） |
| mes_choice.html | 廃止（内部出力に変更） |
| mes_modify.html | 廃止（内部出力に変更） |
